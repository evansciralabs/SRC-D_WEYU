...
const PDEF: [(f32, f32, bool); NP] = [
    ...
    // ═ filter (63-66, pre-distortion TPT SVF) ═
    (20.0, 18000.0, true), (0.0, 1.0, false), (0.0, 3.0, false), (0.0, 1.0, false),
];
...
struct Fx {
    ...
    f_ic1: f32, f_ic2: f32, f_env: f32,      // ◍ TPT SVF integrators + follower
}
...
pub struct RiotCore {
    ...
    clock: f64,
    fx: Fx,
    q_s: [f32; NP], qs_init: bool,           // smoothed param snapshot
    p_cap: f32,                              // smoothed partial cap
    out_buf: [f32; 128],                     // Internal buffer avoids JS allocations
}
...
impl RiotCore {
    ...
    pub fn out_ptr(&self) -> *const f32 { self.out_buf.as_ptr() }
    ...
    /// Render one AudioWorklet quantum.
    /// Internal buffer used to bypass Wasm allocation per block.
    pub fn process(&mut self) {
        ...
        // One-pole param smoothing: fader moves and LFO block-steps land as
        // ~20ms ramps instead of block-edge cliffs — THE broadband click
        // source. Stepped params (shapes/modes) snap. (twin of JS core)
        // ⚠ keep in sync with c:"step" entries in the HTML PARAMS table.
        const STEPPED: [usize; 4] = [36, 40, 59, 65];
        if !self.qs_init { self.q_s = q; self.qs_init = true; }
        for i in 0..NP {
            self.q_s[i] = if STEPPED.contains(&i) { q[i] }
                          else { self.q_s[i] + (q[i] - self.q_s[i]) * 0.35 };
        }
        let q = self.q_s;
        ...
        let (tex_lv, tex_m, grain, det) = (map_p(26, q[26]), map_p(27, q[27]), map_p(28, q[28]), map_p(29, q[29]));
        ...
        let cap_tgt = if active_v <= 3 { 16.0 }
                 else if active_v <= 5 { 12.0 }
                 else if active_v <= 6 { 9.0 } else { 7.0 };
        // Glide the cap and FADE the boundary partial (twin of JS core):
        // hard p_top jumps gated full-weight partials on voice-count changes.
        self.p_cap += (cap_tgt - self.p_cap) * 0.08;
        let p_cap_f = self.p_cap;
        let p_top = p_max.min(p_cap_f.ceil() as usize);
        ...
        for vi in 0..(NVOICE + NTAIL) {
            ...
            for s in 0..n {
                ...
                for p in 0..p_top {
                    ...
                    let nyq = self.sr * 0.48;
                    if fn_ >= nyq { continue; }
                    // alias FADE, not gate — taper over the top 15% so a
                    // partial crossing the guard never hard-toggles (twin of JS)
                    let ag = if fn_ > nyq * 0.85 { (nyq - fn_) / (nyq * 0.15) } else { 1.0 };
                    let cg = (p_cap_f - p as f32).clamp(0.0, 1.0);
                    v.ph_a[p] += fn_ * dt; if v.ph_a[p] >= 1.0 { v.ph_a[p] -= 1.0; }
                    add += wp * ag * cg * (TAU * v.ph_a[p]).sin();
                }
                add *= w_norm;
                ...
                if tex_lv > 0.001 {
                    ...
                    // SYMMETRIC grain fade (twin of JS): fade OUT before the
                    // random jump, IN after — the old half-fix cut the dying
                    // grain instantly → HF click every retrigger.
                    let g_g = if grain_len > 0 {
                        let g_out = if v.g_cnt < 384 { v.g_cnt as f32 / 384.0 } else { 1.0 };
                        v.g_fade.min(g_out)
                    } else { 1.0 };
                    v.ph_t += f0 * dt; if v.ph_t >= 1.0 { v.ph_t -= 1.0; }
                    v.ph_t2 += f0 * (1.0 + det * 0.012) * dt; if v.ph_t2 >= 1.0 { v.ph_t2 -= 1.0; }
                    let h1 = self.read_table(fi, ff, v.ph_t) * g_g;
                    let h2 = self.read_table(fi, ff, v.ph_t2);
                    tex = (h1 + h2) * 0.5;
                }

                self.out_buf[s] += (sub * sub_lv + add * add_lv + tex * tex_lv) * v.env * vel * 0.35;
                v.t += dt;
            }
            if vi < NVOICE { self.voices[vi] = v; } else { self.tails[vi - NVOICE] = v; }
        }

        self.process_fx(&q);                    // ═ FX RACK (see below) ═
        for s in self.out_buf.iter_mut() { *s = (*s * master * 1.4).tanh(); }   // master soft clip
    }
}
...
impl RiotCore {
    ...
    fn process_fx(&mut self, q: &[f32; NP]) {
        let n = 128;
        let sr = self.sr;
        let dt = 1.0 / sr;
        let sr_k = sr / 48000.0;
        ...
        // ◍ filter: LP fully open with env centered = true bypass (bit-identical)
        let f_cut_n = q[63].clamp(0.0, 1.0);
        let f_res = map_p(64, q[64]);
        let f_mode = map_p(65, q[65]).round() as u32;
        let f_env_a = (map_p(66, q[66]) - 0.5) * 2.0;
        let flt_on = !(f_mode == 0 && f_cut_n > 0.999 && f_env_a.abs() < 0.004);
        let dist_on = d_mix > 0.001;
        let anlg_on = p_mix > 0.001 || c_mix > 0.001 || t_dep > 0.001;
        let hyb_on  = y_mix > 0.001 || v_mix > 0.001;
        if !flt_on && !dist_on && !anlg_on && !hyb_on { return; }

        // Denormal flush: subnormal floats in decaying feedback state stall
        // ARM float units under wasm's mandatory full-IEEE semantics →
        // periodic CPU-spike pops in quiet tails. (twin of JS core)
        {
            let fl = |v: f32| if v > -1e-15 && v < 1e-15 { 0.0 } else { v };
            let fx = &mut self.fx;
            fx.dly_lp = fl(fx.dly_lp);
            for k in 0..4 { fx.vlp[k] = fl(fx.vlp[k]); }
            fx.f_ic1 = fl(fx.f_ic1); fx.f_ic2 = fl(fx.f_ic2);
            fx.scr_lp = fl(fx.scr_lp); fx.scr_bp = fl(fx.scr_bp);
            fx.tone_lp = fl(fx.tone_lp); fx.dc_y = fl(fx.dc_y); fx.ic_y = fl(fx.ic_y);
            fx.f_env = fl(fx.f_env); fx.cmp_env = fl(fx.cmp_env); fx.scr_env = fl(fx.scr_env);
            fx.ph_fb = fl(fx.ph_fb);
            for k in 0..6 { fx.ap_x[k] = fl(fx.ap_x[k]); fx.ap_y[k] = fl(fx.ap_y[k]); }
        }
        ...
        // ── filter block consts (TPT SVF — unconditionally stable to Nyquist) ──
        let (mut f_a1, mut f_a2, mut f_a3, mut f_k) = (0.0f32, 0.0f32, 0.0f32, 1.0f32);
        if flt_on {
            let f_eff = (map_p(63, q[63]) * 2f32.powf(f_env_a * self.fx.f_env * 6.0))
                .clamp(20.0, sr * 0.49);
            let g = (core::f32::consts::PI * f_eff / sr).tan();
            f_k = 2.0 - 1.96 * f_res;            // res→1 approaches self-osc
            f_a1 = 1.0 / (1.0 + g * (g + f_k)); f_a2 = g * f_a1; f_a3 = g * f_a2;
        }
        ...
        let f = &mut self.fx;
        for s in 0..n {
            let mut x = self.out_buf[s];

            // ═ ◍ FILTER ═
            if flt_on {
                let ax = x.abs();                // follower feeds next block's cutoff
                f.f_env += (ax - f.f_env) * if ax > f.f_env { 0.03 } else { 0.0015 };
                let v3 = x - f.f_ic2;
                let v1 = f_a1 * f.f_ic1 + f_a2 * v3;
                let v2 = f.f_ic2 + f_a2 * f.f_ic1 + f_a3 * v3;
                f.f_ic1 = 2.0 * v1 - f.f_ic1; f.f_ic2 = 2.0 * v2 - f.f_ic2;
                x = match f_mode {
                    0 => v2,                     // LP
                    1 => f_k * v1,               // BP (unity peak)
                    2 => x - f_k * v1 - v2,      // HP
                    _ => x - f_k * v1,           // NOTCH
                };
            }
            ...
            if dist_on {
                ...
                let dc_o = y - f.dc_x + 0.999 * f.dc_y;   // corner ~7.6Hz: sub survives the dist, infrasonics dont 
                f.dc_x = y; f.dc_y = dc_o;
            ...
            if hyb_on {
                ...
                // write-flush: buffers must never store subnormals
                let dw = x + f.dly_lp * d_fb;
                f.dly[f.dly_w] = if dw > -1e-15 && dw < 1e-15 { 0.0 } else { dw };
                ...
                if v_mix > 0.001 {
                    ...
                    for k in 0..4 {
                        let w = d + vg * m[k];
                        f.vb[k][f.vi[k]] = if w > -1e-15 && w < 1e-15 { 0.0 } else { w };
                        f.vi[k] += 1;
                    }
